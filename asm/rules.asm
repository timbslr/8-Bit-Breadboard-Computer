...
#ruledef {
	...
	ldsp {reg: register} => 0b101110 @ reg
	stsp {reg: register} => 0b101111 @ reg
	li {reg: register}, {imm: i8} => 0b101010 @ reg @ imm
	push {reg: register} => asm{ decsp } @ asm{ stsp {reg} }
	pop {reg: register} => asm{ peek {reg} } @ asm{ incsp }
	peek {reg: register} => asm{ ldsp {reg} }
	incx => 0b01110000
	incsp => 0b00110000
	...
	outlcdic {imm: i8} => 0b010111 @ imm
	outlcdd {reg: register} => 0b011011 @ reg
	outlcdid {imm: i8} => 0b011111 @ imm
	nop => 0b00000000
	hlt => 0b00000001
	call {addr: u16} => 0b00000010 @ le(addr)
	ret => 0b00000011
	jmp {addr: u16} => 0b00000101 @ le(addr)
	beq {addr: u16} => 0b00001000 @ le(addr)
	beqi {imm: i8}, {addr: u16} => asm{ li TMP, {imm} } @ asm{ beq {addr} }
	bne {addr: u16} => 0b00001001 @ le(addr)
	...
	bltu {addr: u16} => 0b00100000 @ le(addr)
	...
	bgeu {addr: u16} => 0b00100001 @ le(addr)
	...
	bcs {addr: u16} => 0b00100010 @ le(addr)
	bcc {addr: u16} => 0b00100011 @ le(addr)
